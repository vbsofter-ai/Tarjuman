...
import { CheckCircle2, Loader2, AlertTriangle, ArrowRight, Globe, ShieldCheck, Unlock } from "lucide-react";
...
export default function BillingSuccessClient({ initialOpenSource }: { initialOpenSource: OpenSourceState }) {
  ...
  useEffect(() => {
    ...
    // Server already provided the initial state. Re-fetch to pick up
    // admin toggles that happened after the page render.
    ...
    <div className={`min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 flex items-center justify-center p-4 ${isRtl ? "font-tajawal" : "font-sans"}`}>
      <div className="w-full max-w-xl">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-8 sm:p-10 text-center">
          ...
          {!openSourceState?.enabled && status === "verifying" && (
          ...
          {!openSourceState?.enabled && status === "paid" && (
            <div className="space-y-5">
              <div className="w-20 h-20 mx-auto rounded-full bg-emerald-100 flex items-center justify-center shadow-sm">
          ...
          {!openSourceState?.enabled && status === "pending" && (
            <div className="space-y-4">
          ...
          {!openSourceState?.enabled && status === "failed" && (
            <div className="space-y-4">
