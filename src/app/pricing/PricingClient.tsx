...
import { Check, Star, Shield, Zap, Sparkles, Globe, Loader2, AlertTriangle, CreditCard, Unlock, Heart } from "lucide-react";
...
const T = {
  ar: {
    ...
    openSourceFeature3: "تحليل لغوي متقدم + قاموس مخصص",
    openSourceFeature4: "بدون اشتراك أو بطاقة بنكية",
    openSourceFooter: "شكراً لدعمك — ترجمان أصبح متاحاً للجميع كهدية من المجتمع.",
  },
  en: {
    navHome: "Home",
    navPricing: "Pricing",
    ...
    openSourceFeature3: "Advanced linguistic analysis + custom glossary",
    openSourceFeature4: "No subscription or credit card needed",
    openSourceFooter: "Thanks for your support — Tarjuman is freely available as a gift to the community.",
  },
};

export default function PricingClient({ initialOpenSource }: { initialOpenSource: OpenSourceState }) {
  ...
  // Re-fetch public config on focus so admin toggles propagate without full reload.
  // Server already provided the initial state, so this is a fresh update.
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      try {
        ...
        if (cancelled) return;
        if (data && data.openSource) {
          setOpenSourceState({
            enabled: Boolean(data.openSource.enabled),
            message: String(data.openSource.message || ""),
          });
        }
      } catch {
        // Silent
  ...
  const handleSubscribe = async (planId: PlanId) => {
    ...
    // Open Source / Free Mode — no payment required, send user to the app
    if (openSourceState?.enabled) {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 ${isRtl ? "font-tajawal" : "font-sans"}`}>
      ...
      <section className="pt-16 sm:pt-20 pb-8 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            ...
            {openSourceState?.enabled ? t.openSourceBadge : t.badge}
          </span>
          <h1 className="mt-5 text-3xl sm:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">
            {openSourceState?.enabled ? t.openSourceHeroTitle : t.heroTitle}
          </h1>
          ...
          <div
            className={`mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold border ${
              openSourceState?.enabled
      ...
      {/* Open Source / Free Mode banner — replaces payment + plans + guarantee sections when admin toggles it on */}
      {openSourceState?.enabled && (
        <section className="px-4 sm:px-6 pb-12">
          <div className="max-w-4xl mx-auto">
            <div className="relative overflow-hidden rounded-3xl border-2 border-emerald-300 bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-8 sm:p-12 shadow-xl">
              <div className="absolute -top-12 -end-12 w-48 h-48 rounded-full bg-emerald-200/40 blur-3xl pointer-events-none" />
              <div className="absolute -bottom-16 -start-16 w-56 h-56 rounded-full bg-teal-200/40 blur-3xl pointer-events-none" />
              <div className="relative">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-600 text-white text-xs font-extrabold uppercase tracking-wider shadow-md">
                  <Unlock className="w-3.5 h-3.5" />
                  <span>{t.openSourceBadge}</span>
                </div>
                <h2 className="mt-6 text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight leading-tight">
                ...
                <div className="mt-7">
                  <Link
                    href="/"
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-extrabold shadow-lg shadow-emerald-600/20 active:scale-95 transition-all"
                  >
                ...
                <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl">
                  {[
                    t.openSourceFeature1,
                    t.openSourceFeature2,
                    t.openSourceFeature3,
                    t.openSourceFeature4,
                  ].map((f, i) => (
                    <div key={i} className="flex items-start gap-2.5 text-xs">
                      <div className="p-1 bg-emerald-100 text-emerald-700 rounded-md flex-shrink-0 mt-0.5">
                        <Check className="w-3 h-3" />
                      </div>
                      <span className="text-slate-700 leading-relaxed font-semibold">{f}</span>
                    </div>
                  ))}
                </div>
                <p className="mt-8 text-xs text-emerald-800/80 flex items-center gap-2 font-semibold">
                  <Heart className="w-3.5 h-3.5 fill-current" />
                  <span>{t.openSourceFooter}</span>
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Guarantee — only when NOT in open source mode */}
      {!openSourceState?.enabled && (
